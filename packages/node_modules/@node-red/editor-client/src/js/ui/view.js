        ...
        if (showAllLinkPorts !== -1 || mouse_mode != RED.state.JOINING) {
            ...
            nodeEnter.each(function (d, i) {
                ...
                if (d._def.button) {
                    ...
                    //WFE
                    bgBackground.setAttribute("stroke", RED.utils.getNodeColor(d.type, d._def));
                    //EFW
                ...
                //WFE
                mainRect.setAttribute("style", "stroke: " + RED.utils.getNodeColor(d.type, d._def));
                if (d.type === "inject")
                    mainRect.classList.add("inject-node")
                if (d.type === "debug")
                    mainRect.classList.add("debug-node")
                //EFW       
                node[0][0].__mainRect__ = mainRect;
                d3.select(mainRect)
                    .on("mouseup", nodeMouseUp)
                ...
                if (d._def.icon) {
                    ...
                    //WFE
                    icon_shade.setAttribute("style", "fill: " + RED.utils.getNodeColor(d.type, d._def))
                    //EFW
                ...
                title.setAttribute("class", "red-ui-flow-node-title-text");
                title.setAttribute("x", 0);
                title.setAttribute("y", 0);
                nodeContents.appendChild(title);
                var color = RED.utils.getNodeColor(d.type, d._def);
                if (color === "#DDAA99")
                    title.innerHTML = "Subflow";
                else
                    title.innerHTML = "Node: " + d.type;


                //Node Input
                var inputIcon = document.createElementNS("http://www.w3.org/2000/svg", "image");
                inputIcon.setAttribute("class", "red-ui-flow-node-intput-icon");
                inputIcon.setAttribute("width", 18);
                inputIcon.setAttribute("href", "/red/pic/input.svg ");
                nodeContents.appendChild(inputIcon);

                var input = document.createElementNS("http://www.w3.org/2000/svg", "text");
                input.setAttribute("class", "red-ui-flow-node-input-label");
                input.setAttribute("x", 0);
                input.setAttribute("y", 0);
                nodeContents.appendChild(input);
                var inputLabel = d.inputLabels;
                if (inputLabel == undefined)
                    input.innerHTML = "Input";
                else
                    input.innerHTML = "Input " + d.inputLabels;
                //Node Output
                var outputIcon = document.createElementNS("http://www.w3.org/2000/svg", "image");
                outputIcon.setAttribute("class", "red-ui-flow-node-output-icon");
                outputIcon.setAttribute("width", 18);
                outputIcon.setAttribute("href", "/red/pic/output.svg");
                nodeContents.appendChild(outputIcon);

                var output = document.createElementNS("http://www.w3.org/2000/svg", "text");
                output.setAttribute("class", "red-ui-flow-node-output-label");
                output.setAttribute("x", 0);
                output.setAttribute("y", 0);
                nodeContents.appendChild(output);
                var outputLabel = d.outputLabels;
                if (outputLabel == undefined)
                    output.innerHTML = "Output";
                else
                    output.innerHTML = "Output " + d.outputLabels;
                //EFW
                node[0][0].appendChild(nodeContents);
            ...
            node.each(function (d, i) {
                ...
                if (d.dirty) {
                    ...
                    if (d.resize || this.__hideLabel__ !== hideLabel || this.__label__ !== label || this.__outputs__.length !== d.outputs) {
                        ...
                        this.__labelLineCount__ = labelParts.lines.length;
                        //WFE
                        if (hideLabel || d.type === "link in" || d.type === "link out" || d.type === "link call") {
                            d.h = 55;
                        } else if (d.type === "inject" || d.type === "debug") {
                            d.h = Math.max(6 + 24 * labelParts.lines.length, (d.outputs || 0) * 15, 60);
                        } else {
                            d.h = Math.max(6 + 24 * labelParts.lines.length, (d.outputs || 0) * 15, 100);
                        }
                        //EFW
                        this.__hideLabel__ = hideLabel;
                    }
                    ...
                    if (d.resize) {
                        ...
                        if (hideLabel || d.type === "link in" || d.type === "link out" || d.type === "link call") {
                            ...
                            d.w = Math.max(180, 20 * (Math.ceil((labelParts.width + 90 + (d._def.inputs > 0 ? 7 : 0)) / 20)));
                        } else {
                            d.w = Math.max(250, 20 * (Math.ceil((labelParts.width + 90 + (d._def.inputs > 0 ? 7 : 0)) / 20)));
                        }
                        //EFW
                        if (ow !== undefined) {
                            d.x += (d.w - ow) / 2;
                    ...
                    if (mouse_mode != RED.state.MOVING_ACTIVE) {
                        ...
                        if (d._def.icon) {
                            ...
                            //WFE
                            const iconShadeHeight = d.h - 2
                            const iconShadeWidth = 53
                            //EFW                       
