 ...
 **/
 ...
 RED.view = (function() {
    ...
    function init() {
        ...
        //label
        const newPattern = $('<svg width="5000" height="5000"><defs><pattern id="circle-pattern" width="36" height="36" patternUnits="userSpaceOnUse"><circle cx="10.5" cy="10.5" r="0.5" fill="#323273"></circle><circle cx="10.5" cy="14.5" r="0.5" fill="#323273"></circle><circle cx="14.5" cy="10.5" r="0.5" fill="#323273"></circle><circle cx="14.5" cy="14.5" r="0.5" fill="#323273"></circle></pattern></defs><rect id="rect" x="0" y="0" width="100%" height="100%" fill="url(#circle-pattern)" style="opacity: 0.7;"></rect></svg>')
        newPattern.insertBefore("#red-ui-workspace-chart > svg > g")[0]
        ...
        //label
        function animateLinks(n, linkLayer, activeLinks) {
            let link = linkLayer.selectAll(".red-ui-flow-link").data(
                activeLinks,
            ...
            link.each(function (d) {
                ...
                if (n.id === d.source.id && d.target.id) {
                    const circle = document.createElementNS("http://www.w3.org/2000/svg", "circle");
                    circle.setAttribute("id", "c" + d.source.id + d.target.id);
                    circle.setAttribute("cx", "0");
                    circle.setAttribute("cy", "0");
                    circle.setAttribute("r", "4");
                    circle.setAttribute("class", "message-UI");
                    ...
                    mpath.setAttributeNS("http://www.w3.org/1999/xlink", "xlink:href", "#path" + d.source.id + d.target.id);
        
                    animateMotion.appendChild(mpath);
                    circle.appendChild(animateMotion);
                    $(".red-ui-workspace-chart-event-layer")[0].appendChild(circle);
                    animateMotion.beginElement();
        
                    setTimeout(() => $("#c" + d.source.id + d.target.id)[0].remove(), 1000);
                }

            });
        }
        RED.comms.subscribe("/sendMessage", function(m, n) {
            animateLinks(n, linkLayer, activeLinks);
        });
            ...
            drop: function (event, ui) {
                ...
                try {
                    ...
                    if (hideLabel) {
                        ...
                        nn.w = Math.max(
                            node_width,
                            20 * (Math.ceil((labelParts.width + 50 + (nn._def.inputs > 0 ? 7 : 0)) / 20))
                        );
                ...
                if (window.customZoomState) {
                    const { scale, transX, transY } = window.customZoomState;
                    mousePos[0] = (mousePos[0] - transX) / scale;
                    mousePos[1] = (mousePos[1] - transY) / scale;
                } else {
                ...
                if (nn.x < minX) nn.x = minX;
                if (nn.y < minY) nn.y = minY;
                if (nn.x > maxX) nn.x = maxX;
                if (nn.y > maxY) nn.y = maxY;
                ...
                if (group && typeof RED.group !== "undefined" && typeof RED.group.addToGroup === "function") {
                ...
                if (RED.view && typeof RED.view.exitActiveGroup === 'function') {
                    RED.view.exitActiveGroup();
                }
                ...
                if (group) {
                    if (RED.view && typeof RED.view.selectGroup === 'function') {
                        RED.view.selectGroup(group, false);
                    }
                    if (RED.view && typeof RED.view.enterActiveGroup === 'function') {
                        RED.view.enterActiveGroup(group);
                    }
    ...
    function canvasMouseMove() {
        ...
        if (mouse_mode == RED.state.JOINING || mouse_mode === RED.state.QUICK_JOINING) {
            ...
            for (i=0;i<drag_lines.length;i++) {
                ...
                var portY = (-((numOutputs-1)/2)*13 +13*sourcePort)+10;
    ...
    function zoomZero() { zoomView(0.9); }
    ...
    function nodeMouseDown(d) {
        ...
        //hossein open node setting by dblclick on node
    ...
    function redrawStatus(d,nodeEl) {
        ...
        if (nodeEl) {
            ...
            if (!showStatus || !d.status || d.d === true) {
                ...
                //label
                nodeEl.__statusGroup__.classList.add("show-status");
                document.querySelectorAll(".show-status").forEach(element => {
                    element.parentNode.classList.add("node-with-status");

                })  
                if(nodeEl.__nodeRect__){  
                    nodeEl.__nodeRect__.setAttribute('style', `
                            width: ${d.w-4}px;
                            height: ${d.h+5}px;
                    `);      
                    nodeEl.__nodeUI_foreign__.setAttribute('height', d.h+73);                                                                        
                }
                ...
                if (d.status.shape == null && fill == null) {
                    ...
                    nodeEl.__statusGroup__.setAttribute("transform","translate(-14,"+(d.h)+")");
                    ...
                    //         height: ${d.h-36}px;
                    // `);      
                    // nodeEl.__nodeUI_foreign__.setAttribute('height', d.h+40);   

                } else {
                    nodeEl.__statusGroup__.setAttribute("transform","translate(20,"+(d.h+9)+")");
        ...
        if (showAllLinkPorts !== -1 || mouse_mode != RED.state.JOINING) {
            ...
            if (activeSubflow) {
                ...
                outGroup.each(function(d,i) {
                    ...
                    port.setAttribute("rx",6);
                    port.setAttribute("ry",6);
                    port.setAttribute("width",12);
                ...
                subflowOutputs.each(function(d,i) {
                    if (d.dirty) {
                        ...
                        if (mouse_mode != RED.state.MOVING_ACTIVE) {
                            ...
                            if(this.__nodeRect__){
                                this.__nodeRect__.classList.toggle("red-ui-flow-node-highlighted",!!d.highlighted );
                            }
            ...
            nodeEnter.each(function(d,i) {
                ...
                if(d.type !== "unknown"){
                    var nodeCategory = d._def.category;
                ...
                function createNodeUI(){
                    ...
                    foreignObject.setAttribute('transform', 'translate(-29, 0)');
                    foreignObject.setAttribute('class', 'node-UI'+(hideLabel?" hide":""));
                    ...
                    div.setAttribute("xmlns", "http://www.w3.org/1999/xhtml");
                        ...
                        data.setAttribute('class',"group");
                        data.setAttribute("category", rootCategory);
                        data.setAttribute("type", d.type);
                        // data.innerHTML =`<div class=overlap-group><div class=rectangle></div><img class=img src=dynamicsrc/images/nodes/common/rectangle-3.svg><div class=text-wrapper>${d.id}</div><p class=div>Node: ${d.type}<div class=text-wrapper-2>Message</div><div class=rectangle-2></div><img class=vector src=${RED.utils.getNodeIcon(d._def, d)}><div class=rectangle-3></div><img class=img-2 src=dynamicsrc/images/nodes/common/group-140.png> <img class=img-3 src=dynamicsrc/images/nodes/common/group-138.png></div>`
                          const overlapGroup = document.createElement("div");
                          overlapGroup.className = "overlap-group";
                          
                          //hossein create Notification For Nodes
                          const Notification = document.createElement("div");
                          Notification.className = "node-Notification";
                          node[0][0].__nodeNotification__ = Notification;
                          overlapGroup.appendChild(Notification);
                          
                          
                          const NodeInform = document.createElement("div");
                          NodeInform.className = "node-Inform";
                          Notification.appendChild(NodeInform);
                          
                          
                          const NodeError = document.createElement("div");
                          NodeError.className = "node-error-notification";
                          Notification.appendChild(NodeError);
                          
                          
                          const ImageInform = document.createElement("img");
                          ImageInform.className = "image-Inform";
                          ImageInform.src = "dynamicsrc/images/nodes/common/info-icon.svg"
                          NodeInform.appendChild(ImageInform);
                          
                          const MessageInform = document.createElement("p");
                          MessageInform.className = "message-Inform";
                          MessageInform.textContent = "This node has not been deployed yet."
                          NodeInform.appendChild(MessageInform);
                          
                          const ImageError = document.createElement("img");
                          ImageError.className = "image-Error";
                          ImageError.src = "dynamicsrc/images/nodes/common/Error.svg"
                          NodeError.appendChild(ImageError);
                          
                          const MessageError = document.createElement("p");
                          MessageError.className = "message-Error";
                          MessageError.textContent = "This node has an error."
                          NodeError.appendChild(MessageError);
                          //hossein create Notification For Nodes
                        ...
                        //   overlapGroup.appendChild(rectangle);
                          ...
                          rectangle3.className = "rectangle-3";
                          overlapGroup.appendChild(rectangle3);
                          node[0][0].__nodeRect__ = rectangle3;
                          
                          const status = document.createElement("div");
                          status.className = "node-status-section";
                          overlapGroup.appendChild(status);


                          if(d.type === "inject"){
                            const bottom = document.createElement("div");
                            bottom.className = "node-bottom-section";
                            node[0][0].__nodeBottom__ = bottom;
                            overlapGroup.appendChild(bottom);
                          }
                          ...
                          node[0][0].__nodeTitle__ = textWrapper;
                          overlapGroup.appendChild(textWrapper);
                        
                          const para = document.createElement("p");
                          para.className = "div";

                          var nodeType 
                          if(!d.type.includes("subflow")){
                            nodeType=d.type
                          }else{
                            nodeType="subflow"
                          }
                          para.textContent = `Node: ${nodeType}`;
                          overlapGroup.appendChild(para);
                        
                          const rectangle2 = document.createElement("div");
                          rectangle2.className = "rectangle-2";
                          overlapGroup.appendChild(rectangle2);


                          if(d.type === "inject"){
                            const separator = document.createElement("div");
                            separator.className = "separator";
                            overlapGroup.appendChild(separator);
                          }
                        
                          const vector = document.createElement("img");
                          vector.className = "vector";
                          vector.src = RED.utils.getNodeIcon(d._def, d); // Assuming RED is globally available
                          overlapGroup.appendChild(vector);
                        
                          data.appendChild(overlapGroup);
                        ...
                        foreignObject.appendChild(div);
                            ...
                            nodeContents.appendChild(foreignObject);

                        node[0][0].__nodeUI__ = data;
                        node[0][0].__nodeUI_foreign__ = foreignObject; 
                        
                        
                    }
                    
                    if(d.type !== "link in" && d.type !== "link out" && d.type !== "link call"){
                        createNodeUI()
                    }
                ...
                mainRect.setAttribute("type", d.type);
                ...
                //label
                function nodeToolBar(d){
                    
                    var foreignObject = document.createElementNS('http://www.w3.org/2000/svg', 'foreignObject');
                    foreignObject.setAttribute("height", "40px");
                    foreignObject.setAttribute("width", "400px");
                    foreignObject.setAttribute("class", "node-tools"+(hideLabel?" hide":""));
                    ...
                    deleteButton.addEventListener("click", function() {
                        ...
                        // Store the node information before deletion
                        var nodeToRemove = RED.nodes.node(d.id); // Get the node
                        removedNodes.push(nodeToRemove); // Store the node object instead of the id

                        // Manually find the links of the node
                        ...
                        // Delete the node
                        var removedEntities = RED.nodes.remove(d.id);
                        removedNodes = removedNodes.concat(removedEntities.nodes);
                        removedLinks = removedLinks.concat(removedEntities.links);

                        // Register the changes in the history
                        var historyEvent = {
                            t: 'delete',
                            nodes: removedNodes,
                            links: removedLinks,
                            changes: {
                                id: d.id,
                                node: nodeToRemove, // Store node information for restoration
                                links: removedLinks
                            },
                            dirty: startDirty
                        };
                        ...
                        RED.nodes.dirty(true);
                        RED.view.redraw(true);
                        RED.history.push(historyEvent); // Register in history

                        RED.tray.close();
                    });
                    
                      const img0 = document.createElement("img");
                      img0.src = "dynamicsrc/images/nodes/common/code-svgrepo-com.svg";
                      deleteButton.appendChild(img0);
                    ...
                    // Set icon sources
                    img1.src = "dynamicsrc/images/nodes/common/lsicon-disable-outline.svg";
                    img1.className = "disable-icon";
                    
                    img2.src = "dynamicsrc/images/nodes/common/enable.svg";
                    img2.className = "enable-icon";
                    
                    // Check current state of the node and apply correct UI on load
                    if (d.d === true) {
                        statusButton.className = "enable";
                        label.innerText = "enable";
                        img1.style.display = "none";
                        img2.style.display = "inline";
                    } else {
                        statusButton.className = "disable";
                    ...
                    // Assemble button
                    statusButton.appendChild(img1);
                    statusButton.appendChild(img2);
                    statusButton.appendChild(label);
                    
                    // Toggle logic
                    statusButton.addEventListener("click", function () {
                        const escapedId = CSS.escape(d.id);
                        const nodes = document.querySelector(`#${escapedId}`);
                    
                        // Toggle node state in memory
                        ...
                        // Update button UI and node class
                        if (d.d === true) {
                            label.innerText = "enable";
                            statusButton.className = "enable";
                            img1.style.display = "none";
                            img2.style.display = "inline";
                            nodes.classList.add("red-ui-flow-node-disabled");
                        } else {
                            label.innerText = "disable";
                            statusButton.className = "disable";
                            img1.style.display = "inline";
                            img2.style.display = "none";
                            nodes.classList.remove("red-ui-flow-node-disabled");
                        }
                    });
                    ...
                    settingButton.classList = "setting";
                    
                    settingButton.addEventListener("click", function() {
                        
                        if(d.type === "inject"){
                            const escapedId = CSS.escape(d.id); 
                            const doneButton = document.querySelector(`#${escapedId} .done-button button`);
                      ...
                      img3.src = "dynamicsrc/images/nodes/common/vector-3.svg";
                      settingButton.appendChild(img3);
                    ...
                    node[0][0].__nodeToolBar__ = foreignObject;
                ...
                if (d.type === "inject") {
                    ...
                    // First define addItem
                    function addItem(container, nodeId, opt) {
                        const $container = $(container);
                        const prop = opt || { p: "", v: "", vt: "str" };
                    
                        const row = $('<div/>').appendTo($container);
                        row.css({ 'margin-left': '50px', 'margin-top': '20px' });
                        ...
                        const propertyValue = $('<input/>', { class: "node-input-prop-property-value", type: "text" })
                            .css("width", "calc(50% - 30px)")
                            .val(prop.v)
                            .appendTo(row)
                            .typedInput({
                                default: prop.vt || 'str',
                                types: ['flow', 'global', 'str', 'num', 'bool', 'json', 'bin', 'date', 'jsonata', 'env', 'msg']
                            });
                    
                        $(document).on('click', '.red-ui-typedInput-input', function (event) {
                            const input = this;
                            setTimeout(() => {
                        ...
                        $(document).on('keydown', '.red-ui-typedInput-input', function (event) {
                            event.stopPropagation();
                        });
                        ...
                        .on('click', function () {
                                ...
                                const containerHeight = $("#" + nodeId + " .node-input-property-container").height();
                                d.h = containerHeight + 220
                                RED.view.redraw();
                            });

                    }

                    function saveNodeChanges(nodeId, final = false) {
                        const node = RED.nodes.node(nodeId);
                        if (!node) return;

                        const containerSelector = "#" + nodeId + " .node-input-property-container";
                        const nameInputs = $(containerSelector).find('.node-input-prop-property-name');
                        const valueInputs = $(containerSelector).find('.node-input-prop-property-value');
                        ...
                        for (let i = 0; i < nameInputs.length; i++) {
                            ...
                            const name = $(nameInputs[i]).typedInput('value').trim();
                            const value = $(valueInputs[i]).typedInput('value');
                            const vt = $(valueInputs[i]).typedInput('type');

                            if (!name) {
                                if (final) {
                                    
                                    $(nameInputs[i]).closest('div').remove();
                                    const containerHeight = $("#" + nodeId + " .node-input-property-container").height();
                                    d.h = containerHeight + 220;
                                }
                                continue;
                    ...
                    function nodeDoneButton(nodeId) {
                        var foreignObject = document.createElementNS('http://www.w3.org/2000/svg', 'foreignObject');
                        foreignObject.setAttribute("height", "28px");
                        foreignObject.setAttribute("width", "70px");
                        foreignObject.setAttribute("class", "done-button");
                        node[0][0].__injectUpdateBtn__ = foreignObject;
                        ...
                        doneButton.style.color = "rgb(255, 255, 255)";
                        doneButton.style.padding = "7px 14px";
                        doneButton.style.border = "none";
                        doneButton.style.cursor = "pointer";
                        doneButton.style.fontWeight = "500";

                        doneButton.addEventListener("click", function () {
                            saveNodeChanges(nodeId, true);

                            setTimeout(() => {
                                const updatedNode = RED.nodes.node(nodeId);
                                if (!updatedNode) return;

                                // const hasTopic = Array.isArray(updatedNode.props) &&
                                //     updatedNode.props.some(p => p.p === "topic");

                                // if (hasTopic) {
                                //     d.w = 400;
                                //     const defaultHeight = 210
                                //     const inputCount = Array.isArray(d.props) ? d.props.length : 0;
                                //     d.h = defaultHeight + inputCount * 44
                                // }
                                
                                const node = RED.nodes.node(d.id);
                                node.changed = true;
                                node.dirty = true;
                                RED.nodes.dirty(true);
                                RED.events.emit("nodes:change", node);
                                ...
                                const inputField = nodeContents.querySelector(".input-field");
                                if (inputField) inputField.remove();

                                nodeToolBar(updatedNode);
                                
                            }, 10);
                        });

                        div1.appendChild(doneButton);
                        foreignObject.appendChild(div1);
                        nodeContents.appendChild(foreignObject);
                    }

                    
                    function nodeInputField(nodeId) {
                        var foreignObject = document.createElementNS('http://www.w3.org/2000/svg', 'foreignObject');
                        node[0][0].__injectItems__ = foreignObject;
                        foreignObject.setAttribute("width", "450px");
                        foreignObject.setAttribute("class", "input-field");
                        foreignObject.style.transform = "translateY(118px) translateX(18px)";
                        ...
                        div2.setAttribute("class", "node-input-property-container");
                        ...
                        <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                          <path d="M8 1C8.27614 1 8.5 1.22386 8.5 1.5V7.5H14.5C14.7761 7.5 15 7.72386 15 8C15 8.27614 14.7761 8.5 14.5 8.5H8.5V14.5C8.5 14.7761 8.27614 15 8 15C7.72386 15 7.5 14.7761 7.5 14.5V8.5H1.5C1.22386 8.5 1 8.27614 1 8C1 7.72386 1.22386 7.5 1.5 7.5H7.5V1.5C7.5 1.22386 7.72386 1 8 1Z"/>
                        </svg>
                      `; 
                        addButton.classList.add("add", "add-property"); 

                        node[0][0].__InjectAddButton__ = addButton;

                        addButton.addEventListener("click", function () {
                            addItem($(div2), nodeId, { p: "", v: "", vt: "str" });
                            saveNodeChanges(nodeId);
                            
                            const containerHeight = $("#" + d.id + " .node-input-property-container").height();
                            d.h = containerHeight + 220
                                
                            // Update resizing Inject Node
                            RED.view.redraw();
                        });
                        ...
                        const defNodeType = RED.nodes.getType("inject");
                        const defaultProps = Array.isArray(defNodeType?.defaults?.props)
                            ? defNodeType.defaults.props.map(p => ({
                                p: p.p || "",
                                v: p.v || "",
                                vt: p.vt || "str"
                            }))
                        ...
                        if (d.props.length === 0 && defaultProps.length > 0) {
                            d.props = JSON.parse(JSON.stringify(defaultProps));
                        }
                
                        const propsToRender = d.props.map(prop => {
                            const p = prop.p || "";
                            let v = prop.v;
                            let vt = prop.vt;
                
                            if (p === "payload") {
                                v = v !== undefined ? v : (d.payload !== undefined ? d.payload : "");
                                vt = vt || d.payloadType || "str";
                            } else if (p === "topic") {
                                v = v !== undefined ? v : (d.topic !== undefined ? d.topic : "");
                                vt = vt || "str";
                            } else {
                                v = v !== undefined ? v : "";
                                vt = vt || "str";
                        ...
                        propsToRender.forEach(prop => addItem($(div2), nodeId, prop));
                    }
                ...
                function createNodeButtons(){
                    var foreignObject = document.createElementNS('http://www.w3.org/2000/svg', 'foreignObject');
                    foreignObject.setAttribute("height", "28px");
                    foreignObject.setAttribute("width", "32px");
                    foreignObject.setAttribute("class", "node-button");
                    ...
                    if(d.type === 'inject'){
                        ...
                        buttonElement.classList.add('btn', 'relative', 'h-7', 'w-7', 'rounded-full', 'p-0', 'hover:bg-slate-300/20', 'focus:bg-slate-300/20', 'active:bg-slate-300/25', 'dark:hover:bg-navy-300/20', 'dark:focus:bg-navy-300/20', 'dark:active:bg-navy-300/25');

                        // Create the SVG element inside the button
                        const svgElement = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
                        svgElement.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
                        svgElement.setAttribute('width', '25');
                        svgElement.setAttribute('height', '25');
                        svgElement.setAttribute('fill', '#6261a5');
                        svgElement.classList.add('bi', 'bi-play');
                        svgElement.setAttribute('viewBox', '0 0 16 16');
                        svgElement.setAttribute('id', 'IconChangeColor');

                        // Create the path element inside the SVG
                        const pathElement = document.createElementNS('http://www.w3.org/2000/svg', 'path');
                        pathElement.setAttribute('d', 'M10.804 8 5 4.633v6.734L10.804 8zm.792-.696a.802.802 0 0 1 0 1.392l-6.363 3.692C4.713 12.69 4 12.345 4 11.692V4.308c0-.653.713-.998 1.233-.696l6.363 3.692z');
                        pathElement.setAttribute('id', 'mainIconPathAttribute');
                        pathElement.setAttribute('stroke-width', '0');
                        pathElement.setAttribute('stroke', '#ff0000');
                        pathElement.setAttribute('style', 'transform: translate(4px, 0px);');
                        ...
                        innerSpan1.classList.add('absolute', '-top-px', '-right-px', 'flex', 'h-3', 'w-3', 'items-center', 'justify-center', 'inject-button');
                        innerSpan1.setAttribute('style', 'transform: translate(-2px, 5px);');

                        const innerSpan2 = document.createElement('span');
                        innerSpan2.classList.add('absolute', 'inline-flex', 'h-full', 'w-full', 'animate-ping', 'rounded-full', 'bg-success', 'opacity-80');

                        const innerSpan3 = document.createElement('span');
                        innerSpan3.classList.add('inline-flex', 'h-2', 'w-2', 'rounded-full', 'bg-success');

                        // Append the inner span elements to the outer span
                        innerSpan1.appendChild(innerSpan2);
                        innerSpan1.appendChild(innerSpan3);

                        // Append the inner span to the button
                        buttonElement.appendChild(innerSpan1);
                        ...
                        inputElement.addEventListener("change", function() {
                            bgBackground.dispatchEvent(new Event("click")); // Trigger click event on bgBackground element
                        });

                        // Create tag1 element
                        const tag1 = document.createElement('a');
                        tag1.href = '#';
                        tag1.className = 'w-8 tag btn bg-secondary font-medium text-white hover:bg-secondary-focus focus:bg-secondary-focus active:bg-secondary-focus/90 debug-on';
                        tag1.innerText = 'ON';

                        // Create tag2 element
                        const tag2 = document.createElement('a');
                        tag2.href = '#';
                        tag2.className = 'w-8 tag bg-slate-150 text-slate-800 hover:bg-slate-200 focus:bg-slate-200 active:bg-slate-200/80 dark:bg-navy-500 dark:text-navy-100 dark:hover:bg-navy-450 dark:focus:bg-navy-450 dark:active:bg-navy-450/90 debug-off';
                        tag2.innerText = 'OFF';
                        tag2.style.display = 'none';

                        if(d[d._def.button.toggle]){
                            tag1.style.display = '';
                            tag2.style.display = 'none';
                        }
                        else{
                        ...
                        tag1.addEventListener('click', () => {
                            nodeButtonClicked(d);
                            bgBackground.dispatchEvent(new Event("click"));
                            tag1.style.display = 'none';
                            tag2.style.display = '';
                          });
                        ...
                        // Append tags to the inner div
                        innerDiv.appendChild(tag1);
                        innerDiv.appendChild(tag2);
                    ...
                    foreignObject.appendChild(div);
                    if(d.type === "inject" ||  d.type === "debug"){
                        nodeContents.appendChild(foreignObject);
                    }
                    node[0][0].__NodeButtons__ = foreignObject;
                ...
                if (d.type === "inject") {
                    ...
                    foreignObject.setAttribute("height", "24px");
                    foreignObject.setAttribute("width", "24px");
                    foreignObject.setAttribute("class", "node-button");
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 7 10" fill="none" style="transform: translate(3.5px, 2px);">
                    <path d="M0.965088 6.44238L3.36893 8.84623L5.77278 6.44238" stroke="#A3ADC2" stroke-width="0.961538" stroke-linecap="round" stroke-linejoin="round"></path>
                    <path d="M0.965088 3.55765L3.36893 1.15381L5.77278 3.55765" stroke="#A3ADC2" stroke-width="0.961538" stroke-linecap="round" stroke-linejoin="round"></path>
                    </svg>
                    `;

                    expandButton.addEventListener("click", function () {

                        document.querySelector(`#${escapedId}`).classList.add('expand');

                        const el = document.querySelector(`#${escapedId} .input-field`);
                        if (el) {
                            el.classList.add('show');
                            el.classList.remove('hide');
                        }

                        const nodeBottomSection = document.querySelector(`#${escapedId} .node-bottom-section`);
                        if (nodeBottomSection) {
                            nodeBottomSection.classList.add('show');
                            nodeBottomSection.classList.remove('hide');
                        }

                        const doneButton = document.querySelector(`#${escapedId} .done-button`);
                        if (doneButton) {
                            doneButton.classList.add('show');
                            doneButton.classList.remove('hide');
                        }

                        document.querySelector(`#${escapedId} .shrink-button`).style.display = "block";
                        document.querySelector(`#${escapedId} .expand-button`).style.display = "none";
                        ...
                        //hossein change height of node and save it.
                        const node = RED.nodes.node(d.id);
                        node.changed = true;
                        node.dirty = true;
                        RED.nodes.dirty(true);
                    ...
                    const shrinkButton = document.createElement("div");
                    shrinkButton.setAttribute("class", "shrink-button");
                    shrinkButton.innerHTML = `
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 7 10" fill="none" style="transform: translate(3.5px, 2px);">
                    <path d="M1 8.69238L3.40385 6.28854L5.80769 8.69238" stroke="#A3ADC2" stroke-width="0.961538" stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M1 0.999962L3.40385 3.40381L5.80769 0.999962" stroke="#A3ADC2" stroke-width="0.961538" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>`
                    shrinkButton.addEventListener("click", function () {
                        ...
                        document.querySelector(`#${escapedId}`).classList.remove('expand');

                        const doneButtonClick = document.querySelector(`#${escapedId} .done-button button`);
                        ...
                        setTimeout(() => {
                            ...
                            const nodeBottomSection = document.querySelector(`#${escapedId} .node-bottom-section`);
                            if (nodeBottomSection) {
                                nodeBottomSection.classList.add('hide');
                                nodeBottomSection.classList.remove('show');
                            }
                            ...
                            document.querySelector(`#${escapedId} .shrink-button`).style.display = "none";
                            document.querySelector(`#${escapedId} .expand-button`).style.display = "block";
                            ...
                            var labelParts;
                            labelParts = getLabelParts(label, "red-ui-flow-node-label");
                            d.h = Math.max(6 + 24 * labelParts.lines.length, (d.outputs || 0) * 18, 120);
                            d.w = Math.max(275,20*(Math.ceil((labelParts.width+90+(d._def.inputs>0?7:0))/20)) ); 
                    ...
                    div.appendChild(shrinkButton);

                    foreignObject.appendChild(div);
                    nodeContents.appendChild(foreignObject);
                    node[0][0].__ExpandButton__ = foreignObject;
                }
            ...
            node.each(function(d,i) {
                ...
                if (d.dirty) {
                    ...
                    if (d.resize || this.__hideLabel__ !== hideLabel || this.__label__ !== label || this.__outputs__.length !== d.outputs) {
                        ...
                        } else if (d.type === "link call") {
                            d.h = node_height;
                        }else if (d.type === "inject" && document.getElementById(d.id).classList.contains('expand')) {
                            const defaultHeight = 210
                            const inputCount = Array.isArray(d.props) ? d.props.length : 0;
                            d.h = defaultHeight + inputCount * 44
                        }
                        ...
                        if (this.__textGroup__) {
                            // Create temp text elements if needed
                            while (this.__textGroup__.childNodes.length < labelParts.lines.length) {
                            ...
                            let maxWidth = 0;
                            for (let i = 0; i < labelParts.lines.length; i++) {
                                let lineEl = this.__textGroup__.childNodes[i];
                                lineEl.textContent = labelParts.lines[i];
                                lineEl.setAttribute("x", 0);
                                lineEl.setAttribute("y", i * 24);
                                // Measure text width
                                const width = lineEl.getComputedTextLength();
                                if (width > maxWidth) {
                                    maxWidth = width;
                                }
                            }
                            labelParts.width = maxWidth;
                        } else {
                            labelParts.width = 0;
                        }
                    ...
                    //label
                    if(this.__NodeButtons__){
                            this.__NodeButtons__.setAttribute("transform", "translate("+(d.w-45)+",44)");             
                    }
                    if(this.__ExpandButton__){
                            this.__ExpandButton__.setAttribute("transform", "translate("+((d.w/2)-12)+","+(d.h-17)+")");             
                    }
                    if(this.__nodeUI__){
                            this.__nodeUI__.setAttribute("style", "width:"+d.w+"px; height:"+(d.h-36)+"px;")            
                    }                   
                    if(this.__nodeUI_foreign__ && !this.classList.contains('node-with-status')){
                            this.__nodeUI_foreign__.setAttribute("width", d.w+62);
                            this.__nodeUI_foreign__.setAttribute('height', d.h+40);
                            this.__nodeUI_foreign__.setAttribute("class", "node-UI"+(hideLabel?" hide":""));  
                    }
                    if(this.__nodeRect__ && !this.classList.contains('node-with-status')){
                            this.__nodeRect__.setAttribute('style', `width: ${d.w-4}px; height: ${d.h-36}px;`);
                    }
                    if(this.__nodeNotification__){
                            this.__nodeNotification__.setAttribute('style', `width: ${d.w-2}px;`);
                    }
                    if(this.__nodeBottom__){
                        this.__nodeBottom__.setAttribute('style', `top: ${d.h-75}px;`);
                    }
                    if(this.__nodeToolBar__){
                            this.__nodeToolBar__.setAttribute("width", d.w);
                            this.__nodeToolBar__.setAttribute("class", "node-tools"+(hideLabel?" hide":""));   
                    }
                    //hossein New Inject Dynamic
                    if (this.__injectItems__) {
                        const height = d.h - 158;
                        if (height > 0) {
                            this.__injectItems__.setAttribute('height', height);
                        }
                    }

                    if(this.__injectUpdateBtn__){
                        this.__injectUpdateBtn__.setAttribute('transform', `translate(${300}, ${d.h - 42})`);
                    }
                    if (this.__InjectAddButton__) {
                        // const top = 3;  
                    
                        // this.__InjectAddButton__.setAttribute('style', `
                        //     position: absolute;
                        //     top: ${top}px;
                        //     left: 10px;
                        //     color: white;
                    ...
                    if (mouse_mode != RED.state.MOVING_ACTIVE) {
                        ...
                        if (labelParts) {
                            ...
                            for (var i=0; i<sn; i++) {
                                ...
                                textLines[i].textContent = sa[i];
                                let capitalizedString = sa[i].charAt(0).toUpperCase() + sa[i].slice(1);
                                if(this.__nodeTitle__){
                                    this.__nodeTitle__.innerText = capitalizedString;
                                }
                        ...
                        if ((!isLink || (showAllLinkPorts === -1 && !activeLinkNodes[d.id])) && d.inputs === 0 && !inputPorts.empty()) {
                            ...
                            if (d.type === "link in") {
                                ...
                                inputGroupPorts = inputGroup.append("rect").attr("class","red-ui-flow-port").attr("rx",6).attr("ry",6).attr("width",12).attr("height",12)
                            }
                        ...
                        for(var portIndex = 0; portIndex < numOutputs; portIndex++ ) {
                            ...
                            if (portIndex === this.__outputs__.length) {
                                ...
                                if (d.type === "link out") {
                                    ...
                                    portPort.setAttribute("rx",6);
                                    portPort.setAttribute("ry",6);
                                    portPort.setAttribute("width",12);
                                    portPort.setAttribute("height",12);
                        ...
                        //label                
                        if (d.changed) {
                            $("#" + d.id).addClass("node-changed");
                        } else {
                            $("#" + d.id).removeClass("node-changed");
                        }
                        if (!d.valid) {
                            $("#" + d.id).addClass("node-error");
                        } else {
                            $("#" + d.id).removeClass("node-error");
                        }
            ...
            linkEnter.each(function(d,i) {
                ...
                //label
                pathOutline.setAttribute("id", "path" + d.source.id + d.target.id);
    ...
    return {
        ...
        //hossein New Inject
        getNodePropsFromView: function (nodeId) {
            const scope = document.querySelector(`g.nodegroup[data-node-id="${nodeId}"]`) || document;
            const nameInputs = scope.querySelectorAll('.node-input-prop-property-name');
            const valueInputs = scope.querySelectorAll('.node-input-prop-property-value');
            
            const props = [];
            for (let i = 0; i < nameInputs.length; i++) {
                const name = $(nameInputs[i]).typedInput('value');
                const value = $(valueInputs[i]).typedInput('value');
                const vt = $(valueInputs[i]).typedInput('type');
                if (name !== '') {
                    props.push({ p: name, v: value, vt });
