...
if (parsedArgs.settings) {
    ...
    if (fs.existsSync(path.join(process.env.NODE_RED_HOME,".config.json"))) {
        ...
        var userJSFile = path.join(userDir,"guided-script.js");
        var userStyleFile = path.join(userDir,"guided-style.css");
        if (fs.existsSync(userSettingsFile)) {
            ...
            // Default JS for WFE
            var defaultJS = path.join(__dirname,"guided-script.js");
            var jsStat = fs.statSync(defaultJS);
            if (jsStat.mtime.getTime() <= jsStat.ctime.getTime()) {
                // Default JS file has not been modified - safe to copy
                fs.copySync(defaultJS,userJSFile);
                jsFile = userJSFile;
            } else {
                // Use default guided-script.js as it has been modified
                jsFile = defaultJS;
            }
            // Default CSS for WFE
            var defaultStyle = path.join(__dirname,"guided-style.css");
            var styleStat = fs.statSync(defaultStyle);
            if (styleStat.mtime.getTime() <= styleStat.ctime.getTime()) {
                // Default Style file has not been modified - safe to copy
                fs.copySync(defaultStyle,userStyleFile);
                styleFile = userStyleFile;
            } else {
                // Use default guided-style.css as it has been modified
